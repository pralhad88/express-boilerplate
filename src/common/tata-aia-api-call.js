...
const makeTataAiaApiCall = async (apiPath, method, params, bodyData, token) => {
  ...
  switch (method.toLowerCase()) {
    ...
    case "post":
      try {
        apiCall = await axios.post(url, bodyData, {params: newParams, headers: header})
        if (apiCall.data.status == 200) {
          return apiCall.data
        } else {
          return {
    ...
    case "delete":
      try {
        apiCall = await axios.delete(url, { params: newParams, headers: header, data: bodyData,})
        if (apiCall.data.status == 200) {
          return apiCall.data
        } else {
          return {
    ...
    case "put":
      try {
        apiCall = await  axios.put(url, bodyData, { params: newParams, headers: header })
        if (apiCall.data.status == 200) {
          return apiCall.data
        } else {
          return {
